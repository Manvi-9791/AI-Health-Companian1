  ...
  async generateContent(prompt: string): Promise<string> {
    try {
      ...
      if (!response.data.candidates || response.data.candidates.length === 0) {
        throw new Error('No response from API. The API might have blocked the content due to safety settings.');
      }
      ...
      if (error.response) {
        ...
        if (statusCode === 400 && errorMessage?.includes('API key')) {
          throw new Error('Invalid API key. Please check your Gemini API key in src/config/api.ts or set VITE_GEMINI_API_KEY environment variable.');
        } else if (statusCode === 403) {
          throw new Error('API key authentication failed. Please verify your API key is correct and has proper permissions.');
        ...
        throw new Error('No response from API. Please check your internet connection and API key configuration.');
      } else {
